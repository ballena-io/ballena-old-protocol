...
contract PancakePair is IPancakePair00, PancakeERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint rootK = Math1.sqrt(uint(_reserve0).mul(_reserve1));
                uint rootKLast = Math1.sqrt(_kLast);
                if (rootK > rootKLast) {
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            liquidity = Math1.sqrt(amount0.mul(amount1)).sub(MINIMUM_LIQUIDITY);
           _mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity = Math1.min(amount0.mul(_totalSupply) / _reserve0, amount1.mul(_totalSupply) / _reserve1);
        }
